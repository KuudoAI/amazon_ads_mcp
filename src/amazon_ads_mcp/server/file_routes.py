...
def register_file_routes(server) -> None:
    ...
    @server.custom_route("/downloads/p/{profile_id}/{file_path:path}", methods=["GET"])
    async def download_file(request: Request) -> Response:
        """Download a file by its profile-scoped path.

        The profile_id is embedded in the URL (generated by get_download_url)
        so this endpoint does not depend on MCP session ContextVars. This
        makes it safe for browser/curl follow-ups across sessions.

        Path Parameters:
            profile_id: Amazon Ads profile ID (numeric)
            file_path: Relative file path within the profile's directory
        ...
        # 2. Validate profile_id format (must be numeric)
        if not re.match(r"^\d+$", profile_id):
            return _create_error_response(
                error="Invalid profile ID format",
                error_code="INVALID_PROFILE",
                status_code=400,
                hint="Profile ID must be numeric",
            )
    ...
    @server.custom_route("/downloads/p/{profile_id}", methods=["GET"])
    async def list_download_files_scoped(request: Request) -> JSONResponse:
        """List available downloads for a specific profile.
        ...
        """
        return await _list_downloads_for_profile(request, request.path_params["profile_id"])

    @server.custom_route("/downloads", methods=["GET"])
    async def list_download_files(request: Request) -> JSONResponse:
        """List available downloads using session context.

        Falls back to ContextVar-based profile resolution. Use
        /downloads/p/{profile_id} for session-independent access.
        ...
        if not profile_id:
            return _create_error_response(
                ...
                status_code=401,
                hint="Use /downloads/p/{profile_id} for direct access, or set active profile via MCP",
            )

        return await _list_downloads_for_profile(request, profile_id)
    ...
    @server.custom_route("/downloads/p/{rest:path}", methods=["OPTIONS"])
    async def downloads_cors_preflight_scoped(request: Request) -> Response:
        """Handle CORS preflight for /downloads/p/{profile_id}/..."""
        response = Response(status_code=204)
        return _add_cors_headers(response)
...
async def _list_downloads_for_profile(
    request: Request, profile_id: str
) -> JSONResponse:
    """List downloads for a specific profile.

    Shared implementation used by both the session-based and
    profile-scoped list endpoints.
    """
    auth_error = await _verify_download_auth(request)
    if auth_error is not None:
        return auth_error
