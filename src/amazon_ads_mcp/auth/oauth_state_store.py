...
The rule's own recommendation (CodeQL docs for
``py/weak-sensitive-data-hashing``) explicitly endorses this algorithm
choice: *"Ensure that you use a strong, modern cryptographic hash
function: ... such as SHA-2, or SHA-3 in other cases."* HMAC-SHA256 is
HMAC built on SHA-2; this is the recommended pattern verbatim for
non-password sensitive-data hashing. If the alert reappears after the
rename, dismiss it as **won't fix — false positive**: no password is
hashed, the signing key is a 256-bit value generated by
``secrets.token_hex(32)`` (or operator-supplied via env var), and the
data being MAC'd is server-emitted state plus a random nonce.
"""
